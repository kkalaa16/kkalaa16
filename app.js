...
class FluidBlobField {
  ...
  #render(now) {
    ...
    for (const blob of this.blobs) {
      const pulseScale = this.reducedMotion ? 0.15 : blob.wobble;
      const pulse = 1 + Math.sin(now * 0.0006 + blob.phase) * pulseScale;
...
function setupGlassTilt(selector = ".glass-card") {
